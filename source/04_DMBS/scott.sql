...
-- (1) BETWEEN A AND B : A부터 B까지(A, B포함. A<=B)
    ...
    SELECT * FROM EMP 
        WHERE TO_CHAR(HIREDATE, 'RR/MM/DD') BETWEEN '81/03/01' AND '82/05/31';
...
-- (3) 필드명 LIKE '패턴' : %(0글자이상), _ (한글자)를 포함하는 패턴
    ...
    -- ex. 1월에 입사한 사원
    SELECT * FROM EMP WHERE TO_CHAR(HIREDATE, 'MM') LIKE '__/01/__';
    -- ex. 이름에 %가 들어간 사원
...
SELECT * FROM EMP ORDER BY SAL DESC; -- 내림차순
    ...
    -- ex. 급여가 2000초과하는 사원을 출력 - 이름abc순(오름차순)
...
SELECT ENAME, JOB, HIREDATE
    FROM EMP
    WHERE TO_CHAR(HIREDATE, 'RR/MM/DD') BETWEEN '81/02/20' AND '82/05/01' ORDER BY HIREDATE;
...
SELECT ENAME, SAL  FROM EMP WHERE ENAME >= 'C' AND ENAME <='P' ORDER BY ENAME, SAL;
