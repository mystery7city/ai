...
- 이미지는 EasyOCR 우선 → 실패/예외 시 pytesseract 폴백
...
def _ocr_image_tesseract(img, lang: str = "kor+eng") -> str:
    ...
    [FIX] 들여쓰기 버그 수정 - 이전 버전에서는 return ""가 잘못 중첩되어 
    실제 pytesseract 호출이 실행되지 않았음.
    """
    if not (PYTESSERACT_AVAILABLE and pytesseract):
        return ""
    if not PIL_AVAILABLE or Image is None:
        return ""
    # [FIX] 이 줄이 if 블록 밖으로 나와야 정상 실행됨
    return (pytesseract.image_to_string(img, lang=lang) or "").strip()


def _ocr_with_fallback(img, reader, prefer_easyocr: bool = True) -> Tuple[str, str]:
    """
    OCR 수행 with fallback.
    
    처리 순서:
    1. prefer_easyocr=True이고 reader가 있으면 EasyOCR 시도
    2. EasyOCR이 빈 결과를 반환하거나 예외 발생 시 Tesseract로 fallback
    3. 둘 다 실패하면 빈 문자열 반환
    
    Returns:
        Tuple[str, str]: (추출된 텍스트, 사용된 OCR 엔진 이름)
    """
    text = ""
    engine_used = "none"
    
    # 1) EasyOCR 시도
    if prefer_easyocr and reader is not None:
        try:
            text = _ocr_image_easyocr(img, reader, detail=False)
            if text:
                engine_used = "easyocr"
                logger.debug(f"EasyOCR 성공: {len(text)} chars")
        except Exception as e:
            logger.warning(f"EasyOCR 예외 발생, Tesseract로 fallback: {e}")
            text = ""
    
    # 2) EasyOCR 결과가 없으면 Tesseract fallback
    if not text:
        try:
            text = _ocr_image_tesseract(img)
            if text:
                engine_used = "tesseract"
                logger.debug(f"Tesseract 성공: {len(text)} chars")
        except Exception as e:
            logger.warning(f"Tesseract 예외 발생: {e}")
            text = ""
...
def extract_text_from_path(
    ...
    *,
    ...
    if suffix == ".pdf":
        ...
        for img in imgs:
            text, engine = _ocr_with_fallback(img, reader, prefer_easyocr)
            if text:
                chunks.append(text)
                engines_used.add(engine)
        
        detail_str = f"pymupdf->({'/'.join(sorted(engines_used)) or 'none'})"
        return OCRResult(
            mode="image_pdf_ocr", 
