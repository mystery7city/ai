""" 
Unified Hybrid RAG module (Dense + Sparse) for Korean housing lease Q&A.

This file is a best-of fusion of:
- improved_module_cg.py: robust candidate-level dense+sparse fusion, 2-stage case expansion, optional Cohere rerank
- improved_module_cl.py: stronger BM25 (rank_bm25) + optional Kiwi tokenizer, clearer fusion options
- improved_module_ge.py: global-BM25 approach is intentionally NOT used here (it requires preloading all docs)
...
- embeddings (dense retrieval): Upstage SOLAR embedding by default

Hybrid retrieval (Dense + Sparse)
- Dense: PineconeVectorStore similarity_search_with_score (fallback: similarity_search)
- Sparse: BM25 computed on *dense candidates* (no Pinecone sparse index needed)
- Fusion: rank-based RRF (default) or rank_sum (optional weighted is also supported)
...
Optional packages
- rank_bm25 (better BM25)
- kiwipiepy (better Korean tokenization for BM25)
